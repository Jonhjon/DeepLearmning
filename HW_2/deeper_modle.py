...
from Model import SimpleCNN2_deeper, load_datasets, draw_train_loss, compute_and_save_metrics, draw_confusion_matrix 
...
def main():
    ...
    print("\n--- 正在建立模型 ---")
    model = SimpleCNN2_deeper(num_classes=NUM_CLASSES)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # 計算此 Epoch 的平均訓練損失和準確度
        epoch_train_loss = running_train_loss / len(train_dataset)
        epoch_train_accuracy = train_correct_predictions / len(train_dataset)
        ...
        print(f"  [Train] Loss: {epoch_train_loss:.4f} | Accuracy: {epoch_train_accuracy:.4f} ({(epoch_train_accuracy*100):.2f}%)")
    ...
    try:
        compute_and_save_metrics(all_true, all_pred, class_names=class_names, out_csv='deeper_modle_1_Expansion_metrics.csv')
    except Exception as e:
        print(f"產生評估指標時發生錯誤: {e}")

    # 繪製並儲存訓練/驗證損失曲線
    draw_train_loss(train_losses, val_losses, out_path='deeper_modle_1_Expansion_train_val_loss.png')

    # 繪製並儲存混淆矩陣（原始與正規化）
    try:
        draw_confusion_matrix(all_true, all_pred, class_names=class_names, out_path='deeper_modle_1_Expansion_confusion_matrix.png', normalize=False)
        draw_confusion_matrix(all_true, all_pred, class_names=class_names, out_path='deeper_modle_1_Expansion_confusion_matrix_normalized.png', normalize=True)
    except Exception as e:
