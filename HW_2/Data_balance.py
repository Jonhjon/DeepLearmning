...
BASE_DATA_DIR = os.path.join(BASE_DIR, "output_data_balanced")  # 修改此行

# 如果上面指定的路徑找不到，嘗試幾個常見候選路徑（相對於專案根或本檔案目錄）
if not os.path.isdir(BASE_DATA_DIR):
    # 優先嘗試專案子目錄 HW2/Corn_Leaves_Split
    script_dir = os.path.dirname(os.path.abspath(__file__))
    candidate = os.path.join(script_dir, "output_data_balanced")
...
def main():
    ...
    try:
        compute_and_save_metrics(all_true, all_pred, class_names=class_names, out_csv='balance_metrics.csv')
    except Exception as e:
        print(f"產生評估指標時發生錯誤: {e}")
    
    # 繪製並儲存訓練/驗證損失曲線
    draw_train_loss(train_losses, val_losses, out_path='balance_train_val_loss.png')

    # 繪製並儲存混淆矩陣（原始與正規化）
    try:
        draw_confusion_matrix(all_true, all_pred, class_names=class_names, out_path='balance_confusion_matrix.png', normalize=False)
        draw_confusion_matrix(all_true, all_pred, class_names=class_names, out_path='balance_confusion_matrix_normalized.png', normalize=True)
    except Exception as e:
