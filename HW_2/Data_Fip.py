...
def main():
    ...
    try:
        compute_and_save_metrics(all_true, all_pred, class_names=class_names, out_csv='Flip_Rotation30_metrics.csv')
    except Exception as e:
        print(f"產生評估指標時發生錯誤: {e}")

    # 繪製並儲存訓練/驗證損失曲線
    draw_train_loss(train_losses, val_losses, out_path='Flip_Rotation30_train_val_loss.png')

    # 繪製並儲存混淆矩陣（原始與正規化）
    try:
        draw_confusion_matrix(all_true, all_pred, class_names=class_names, out_path='Flip_Rotation30_confusion_matrix.png', normalize=False)
        draw_confusion_matrix(all_true, all_pred, class_names=class_names, out_path='Flip_Rotation30_confusion_matrix_normalized.png', normalize=True)
    except Exception as e:
